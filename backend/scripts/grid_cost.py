...
Golden test 5 measured 0.057 C between 10 mm and 5 mm. Nobody has measured 15 mm or
20 mm, and the ensemble already runs at 20 mm without that number ever being checked.
...
def main() -> None:
    ...
    header = f"{'dx mm':>6} {'cells':>7} {'limit s':>8} {'dt s':>7} " \
             f"{'peak C':>8} {'peak t h':>9} {'dT c-s C':>9} {'wall s':>7}"
    print(header)
    print("-" * len(header))
    ...
    for dx_m in SPACINGS_M:
        ...
        print(f"{dx_m * 1000:>6.0f} {row.cells:>7} {row.limit_s:>8.2f} {row.dt_s:>7.2f} "
              f"{row.peak_core_temp_c:>8.3f} {row.peak_core_time_h:>9.2f} "
    ...
    for dx_m in SPACINGS_M[1:]:
        ...
        print(f"  {dx_m * 1000:>2.0f} mm  peak {d_peak_c:+.3f} C  peak t {d_time_h:+.2f} h  "
              f"{reference.wall_s / row.wall_s:>5.1f}x faster")
